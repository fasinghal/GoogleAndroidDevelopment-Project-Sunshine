...
public class MainActivity extends AppCompatActivity {
    ...
    // COMPLETED (8) Create a method that will get the user's preferred location and execute your new AsyncTask and call it loadWeatherData
        public void loadWeatherData(String userPreferredLocation){

        URL url = NetworkUtils.buildUrl(userPreferredLocation);
            new NetworkAdaptor().execute(url);
        }
    // COMPLETED (5) Create a class that extends AsyncTask to perform network requests
    public class NetworkAdaptor extends AsyncTask<URL, Void, String>{
        ...
        protected String doInBackground(URL... urls) {
            ...
            try{
               weatherData  = NetworkUtils.getResponseFromHttpUrl(requestURL);
            }catch (IOException e){
                e.printStackTrace();
            }
